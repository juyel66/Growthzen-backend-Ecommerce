...
export const checkoutValidationSchema = z.object({
  customerName: z.string({ message: "customerName is required" }).trim().min(2, "customerName must be at least 2 characters").max(150).optional(),
  customerPhone: z.string({ message: "customerPhone is required" }).trim().min(7, "customerPhone must be at least 7 characters").max(30).regex(/^[+0-9][0-9\s-]+$/, "customerPhone must be a valid phone number").optional(),
  customerEmail: z.union([z.string().trim().email("Invalid email address"), z.literal(""), z.null()]).optional(),
  address: z.string({ message: "address is required" }).trim().min(10, "address must be at least 10 characters").max(500).optional(),
  deliveryArea: deliveryAreaSchema,
  paymentMethod: z.enum(["COD", "BKASH", "NAGAD", "SSLCOMMERZ", "STRIPE", "PAYPAL"], {
    message: "paymentMethod must be COD, BKASH, NAGAD, SSLCOMMERZ, STRIPE, or PAYPAL",
